...
class WeatherForecast(db.Model):
    ...
    latitude = db.Column(db.Numeric(40, 20))
    longitude = db.Column(db.Numeric(40, 20))
    updated_date_utc = db.Column(db.DateTime)
    forecast_date_utc = db.Column(db.DateTime)
    air_temperature = db.Column(db.Numeric(40, 20))
    soil_temperature = db.Column(db.Numeric(40, 20))
    volumetric_soil_moisture_content = db.Column(db.Numeric(40, 20))
    rainfall_boolean = db.Column(db.Boolean, default=False)
    precipitation_rate = db.Column(db.Numeric(40, 20))
    relative_humidity = db.Column(db.Numeric(40, 20))
    dew_point_temperature = db.Column(db.Numeric(40, 20))
    pressure_reduced_to_msl = db.Column(db.Numeric(40, 20))
    pressure = db.Column(db.Numeric(40, 20))
    wind_speed = db.Column(db.Numeric(40, 20))
    total_cloud_cover = db.Column(db.Numeric(40, 20))
