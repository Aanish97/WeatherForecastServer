...
class WeatherForecast(db.Model):
    ...
    latitude = db.Column(db.Numeric(50, 25))
    longitude = db.Column(db.Numeric(50, 25))
    updated_date_utc = db.Column(db.DateTime)
    forecast_date_utc = db.Column(db.DateTime)
    air_temperature = db.Column(db.Numeric(50, 25))
    soil_temperature = db.Column(db.Numeric(50, 25))
    volumetric_soil_moisture_content = db.Column(db.Numeric(50, 25))
    rainfall_boolean = db.Column(db.Boolean, default=False)
    precipitation_rate = db.Column(db.Numeric(50, 25))
    relative_humidity = db.Column(db.Numeric(50, 25))
    dew_point_temperature = db.Column(db.Numeric(50, 25))
    pressure_reduced_to_msl = db.Column(db.Numeric(50, 25))
    pressure = db.Column(db.Numeric(50, 25))
    wind_speed = db.Column(db.Numeric(50, 25))
    total_cloud_cover = db.Column(db.Numeric(50, 25))
